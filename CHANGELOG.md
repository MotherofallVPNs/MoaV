...
### Testing
- **`moav test` protocol coverage expanded** — added connectivity tests for the protocols that previously had none, prioritising the two that are **on by default**: `test_ss` (Shadowsocks-2022, decodes the bundle `ss://` SIP002 URI and drives a sing-box outbound over local SOCKS) and `test_xdns` (runs the bundle's own Xray `xdns-direct-config.json` and checks its SOCKS inbound, warn-on-failure since DNS tunnels are slow). Also added `test_masterdns`, `test_wstunnel` (the WireGuard-over-WebSocket path), and `test_gooserelay`, and upgraded `test_telemt` from a bare `nc -z` reachability probe to an actual MTProto/Fake-TLS handshake check. All appear in the human + JSON result summaries.
...
[Unreleased]: https://github.com/shayanb/MoaV/compare/v1.8.5...HEAD
[1.8.5]: https://github.com/shayanb/MoaV/compare/v1.8.4...v1.8.5
