...
### Breaking Changes
- **Re-bootstrap required when upgrading from ≤1.7.7.** Xray v26.5.9 renamed the inbound `clients`/`accounts` key to `users` ([#6083](https://github.com/XTLS/Xray-core/pull/6083)). MoaV's templates were updated, but the generated `configs/xray/config.json` already on disk still uses the old key — Xray will refuse to start with `unknown field "clients"` until the config is regenerated. The xray service is **not** wired to re-run bootstrap automatically (it has no `depends_on: bootstrap`), so a plain `moav build` + `moav restart` is **not** enough. Apply the upgrade in order: `moav build wstunnel telemt xray --no-cache` → `moav bootstrap` (regenerates configs; idempotent, preserves keys + user UUIDs) → `moav regenerate-users` (so existing bundles pick up `XDNS_RESOLVERS`) → `moav start` (recreates containers on the new images; note `moav restart` reuses the old image). `moav update` now detects config-template changes in the pulled diff and prints this exact sequence
...
- **`moav update` post-update guidance is now config-aware** — After a self-update pull, `moav update` diffs the pulled commits for changed server config templates (`configs/**/*.template`). When a template changed, it prints a complete ordered apply sequence — `moav build … --no-cache` → `moav bootstrap` (regenerate stale configs) → `moav regenerate-users` → `moav start` — instead of the previous bare `moav build` hint. This closes two non-obvious gaps that bit the v1.7.8 Xray schema rename: (1) a config-template change needs a re-bootstrap, not just a rebuild, and (2) `moav build` doesn't recreate containers and `moav restart` reuses the old image, so `moav start` (`docker compose up -d`) is required to actually run rebuilt images. Detection is print-only by design — it never auto-rebuilds or restarts a running node
