...
>
> This is a release candidate published for testing. It has **not** been run on a
> long-lived server, and the upgrade path from 1.9.x is **untested**. If people
> depend on your server working, stay on **[v1.9.1](https://github.com/MotherofallVPNs/MoaV/releases/tag/v1.9.1)**,
> which is what `moav.sh` and the install script still serve by default.
>
> Use this on a throwaway VPS, tell us what breaks.

### What this release is

rc.2 is the security and correctness pass on top of rc.1. Nothing here adds
protocols or features — it is 60+ commits of finding out what was quietly broken
and fixing it, plus the test infrastructure that found most of it.

**If you read one thing:** several of these were silent. The DNS tunnel had not
been tested for months because its test looked for a file that no longer existed.
The connectivity suite could pass while testing nothing at all. A dead WireGuard
tunnel and a healthy one looked identical to the container health checks. Those
gaps are closed, which is most of why this candidate exists.

### Security

The two findings worth knowing about, both fixed:

- **`docker-proxy` was reachable from every container.** It is an unauthenticated
  Docker API endpoint, and it sat on the same flat network as every service that
  terminates untrusted internet traffic — sing-box, xray, wireguard, wstunnel and
  the rest. Any one of them being compromised meant host root. It is now isolated
  on an internal network with only the admin dashboard.
- **The admin panel could serve plain HTTP.** If Let's Encrypt had not issued yet,
  it warned and started anyway — sending your dashboard password (the same one
  Grafana uses) in cleartext on a public port. That was the state of every fresh
  install for its first minutes. It now always has a certificate and refuses to
  start without TLS.

Also: server private keys were world-readable and are now `0600`; `.env` is
created `0600`; all four monitoring exporters gave up their Docker socket access;
and an `ADMIN_IP_WHITELIST` set to a CIDR silently denied everyone, which is why
some operators found it did nothing useful.

### Fixes you may have hit

- `moav regenerate-users` failed for **every** user if you had `ENABLE_REALITY=false`
- WireGuard and AmneziaWG peers could be created with an empty public key — the
  peer simply never worked, with no error
- A Mahsanet API key containing `=` was silently truncated, so authentication
  failed in a way that looked like a wrong key
- `moav user add` could exit silently, with no message at all
- A half-written user state file permanently wedged that user; retrying never helped
- `moav` did not run on macOS at all (bash 3.2)

### Testing

`moav test` now genuinely exercises dnstt again, AnyTLS has a live test for the
first time, and the e2e refuses to report success if the protocols were skipped
rather than passed. Container health is asserted every run, so a service that
crash-loops at startup can no longer hide behind a green tick.

### Upgrading

There is nothing to migrate, but the upgrade path itself is untested. If you try
it on a disposable server, `moav doctor` and `moav test <user>` are the two
commands worth running afterwards, and issues/logs are very welcome.
