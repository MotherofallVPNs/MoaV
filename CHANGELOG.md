...
- **`moav` CLI smoke test** (`scripts/cli-smoke-test.sh`) — beyond the protocol tunnels, this exercises the *tool itself* against the live stack: it runs `help`/`version`/`status`/`users`/`profiles`/`cert status`/`logs`/`check`/`doctor`/`net status`/`conduit-offsets status`, a full `user add` → `revoke` lifecycle, `install.sh --help`, and a non-interactive launch of the TUI menu — each with a hang-guard timeout, asserting read/report commands exit clean and diagnostics don't crash. Wired into the e2e workflow as a step after the connectivity tests, so a broken CLI command fails the run.
