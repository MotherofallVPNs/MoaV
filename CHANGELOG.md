...
- **Xray container status showed as "unknown" in the admin UI even though the protocol and kernel were working and clients were connecting via links** (issue [#117](https://github.com/shayanb/MoaV/issues/117), thanks @sacredx72) — `docker-compose.yml` had no `healthcheck:` block for the xray service, so Docker reported an empty `Health.Status` field and the admin UI's container-status mapping rendered that as "unknown". Added a healthcheck mirroring sing-box's pattern (validates binary + config every 30s; matches the sing-box cadence). The initial attempt used `xray test -c`, which is not a real subcommand — the correct invocation is `xray run -test -c` (verified on xray 26.5.9 by @ibeezhan in PR #120 before any operator hit the broken check). Existing installs pick this up on the next `moav update` + `moav restart xray`
