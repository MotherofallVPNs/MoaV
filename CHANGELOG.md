...
- e2e preserves the Let's Encrypt cert volume across runs (was spending an
  ...
  The release-footer and llms.txt link checks no longer fail when GitHub rate-limits an
...
defaults. Keys, users and certificates are untouched.

### Upgrade Notes

`moav update` prints these for you; they are here so they are on the record:

```bash
moav update
moav build admin        # the admin entrypoint is baked into the image
moav regenerate-users   # existing bundles gain the configs and guide sections below
moav start              # recreate containers on the new image
```

- **`moav build admin` is required, not optional.** The admin container no longer
  receives `ADMIN_PASSWORD` from compose — its entrypoint reads `.env` itself —
  so an un-rebuilt image sees no password and the dashboard fail-closes with 503.
- **`moav regenerate-users` is what makes the bundle fixes visible.** Existing
  users' bundles were generated by the old code; they gain the Slipstream and
  MasterDNS configs, and the TrustTunnel and dnstt sections in their guide, only
  after a regenerate.
- **CDN links keep working on existing servers.** `ENABLE_CDN` now ships `false`,
  but when the variable is absent from your `.env` the previous rule still
  applies (CDN is on if `CDN_SUBDOMAIN` is set), so nothing is lost silently. Add
  `ENABLE_CDN=true` to make it explicit.
- **Donated users get fewer protocols than before, deliberately.** Donate mode
  now restricts to the requested set; previously it leaked the operator's full
  protocol list to the recipient network.

### Fixed
- **Slipstream and MasterDNS configs reach user bundles again.** Both services ran, but no bundle on a live server contained either config — including bootstrap-created users, so this was never limited to users added later. `scripts/user-add.sh` runs on the host and gated each generator on `$STATE_DIR/keys/<key>`, but those keys live in the `moav_state` volume and the host `state/` tree has no `keys/` directory at all. Both guards were bare `if` with no `else`, so the configs were skipped in silence and the command still exited 0 — two last-resort transports served to nobody. `bootstrap` already publishes the client-facing material to host-visible paths (which is why dnstt was unaffected: it reads `outputs/dnstt/server.pub`), so those copies are now mirrored into `$STATE_DIR/keys/` and handed to `secure_state_keys()`. The same gap affected the dashboard, whose add path resolved to the same empty host `state/`. Both guards now warn when a protocol is enabled but its key is missing. Existing users need `moav regenerate-users` to pick the configs up.
- **dnstt now ships its server pubkey per user (`dnstt-server.pub`), and the dashboard reads that.** It was the only protocol without a per-user artifact, so both the guide and the dashboard fell back to a server-wide path and claimed dnstt for *every* user — including donated users, who are provisioned with dnstt off: the dashboard listed dnstt beside a bundle that contained none, and the guide showed a DNS-tunnel section for it. Both now key on the user's own file, like every other protocol. Users also get the key they need for their client instead of having to copy it out of the guide.
- **TrustTunnel and dnstt appear in the bundle guide again.** A user reported TrustTunnel being generated and listed in the admin dashboard while its section was missing from `README.html`. The section was keyed on `trusttunnel.txt`, a filename nothing has ever written — the real artifacts are `trusttunnel.toml` and `.json` — so it was hidden in **every bundle ever produced**. The same phantom-filename bug hid the dnstt section (`dnstt-instructions.txt`, also never written); dnstt is server-wide and has no per-user artifact, so it now keys on the server pubkey the guide already embeds. The other twelve protocols were audited and are correct: `anytls` and `gooserelay` were hidden because they are genuinely disabled.
- **`moav migrate-ip` no longer strands TrustTunnel users on the old server.** It rewrote the same non-existent `trusttunnel.txt`, while `trusttunnel.toml` and `.json` — which do carry the server IP — were left untouched. The DNS-family configs key off the domain and were correctly unaffected.
- **Disabled protocols no longer end up in user bundles.** With `ENABLE_TROJAN=false` and `ENABLE_HYSTERIA2=false`, `moav user add` still produced `trojan.txt`, `hysteria2.txt` and both QR images, so bundles advertised inbounds the operator had switched off. The two add paths had drifted: `generate-user.sh` (container/bootstrap) gated Reality, Trojan, Hysteria2 and TrustTunnel on their flags, while `singbox-user-add.sh` — the host path `moav user add` actually runs — gated only Shadowsocks, AnyTLS and XHTTP. That is why it survived: the bootstrap demo user looked right and every user added afterwards did not. TrustTunnel had a second cause, being gated on `configs/trusttunnel/credentials.toml` existing, which survives disabling the protocol.
- **Donate mode now actually restricts what it hands out.** It set `ENABLE_*` without exporting them, and every sub-script re-sources `.env`, so the overrides never reached `singbox-user-add.sh` or `wg-user-add.sh` — which generate almost everything. A bundle requested as `"reality hysteria2"` still shipped Trojan, Shadowsocks, XHTTP, TrustTunnel, XDNS and CDN to the recipient network. `apply_donate_mode()` now lives in `lib/common.sh`, exports its overrides, and is re-applied after each script's own `.env` load. MasterDNS, XDNS and GooseRelay were never in the disable list and are now included; Shadowsocks is list-driven like its sing-box siblings. Documented in `docs/devdocs/DONATE-MODE.md`, including the checklist for adding a protocol.
- **`llms.txt` links resolve.** The six links under "For agents working in this repo" were relative, so against `https://moav.sh/llms.txt` they resolved to `https://moav.sh/AGENTS.md` and 404'd — they only ever worked when the file was read inside the repo. An agent handed the URL followed them and got nothing, with no error to notice. Now absolute. Reported by a community member.

### Added
- **`moav user sub NAME`** — prints the base64 V2Ray subscription that phone apps import (Streisand, v2rayNG, NekoBox, Hiddify). It hands over the bundle's existing `subscription.txt` rather than re-encoding, so there is no second format to drift.
- **A star-history chart in the README**, generated here rather than embedded. On 2026-06-30 GitHub restricted the stargazers endpoint to a repo's own admins and collaborators, so hosted chart services now return a "GitHub restricted access to star data" notice instead of a chart — while still answering HTTP 200, which makes it look fine. `scripts/gen-star-history.py` builds it with our own token and commits a self-contained SVG: no credential handed to a third party, no request leaving GitHub, and immune to the next policy change. Refreshes daily via a PR, never a direct push.
- **The release footer is generated, not hand-written.** `scripts/render-release-footer.sh` renders it from the same `MOAV_URL_*` variables the CLI footer uses, so release notes and `moav help` cannot disagree. The old heredoc had drifted twice: no community links at all (the v2.0.0 release had them only because they were pasted in by hand), no donation link, and a docs list naming six pages after the rewrite shipped fourteen more. `--check` curls every link and fails on non-200, wired into CI — a renamed docs page would otherwise ship a 404 in the footer of every release from then on.

### Changed
- **CDN links are now opt-in via `ENABLE_CDN`, and off by default.** A CDN link only works once the subdomain is actually proxied through Cloudflare, but `.env.example` shipped `CDN_SUBDOMAIN=cdn` with no flag, so every new server generated CDN links whether or not Cloudflare had been set up — handing users a config that cannot connect. Existing servers are unaffected: when the flag is absent the previous rule still applies (CDN is on if `CDN_SUBDOMAIN` is set), so a working CDN is not silently lost on upgrade. `moav doctor dns` skips the CDN section when the flag is off, and when it is on it now checks the record is **proxied** rather than merely resolving — a grey-cloud record points at the origin, so the link degrades to plain VLESS+WS on a second hostname with none of the resistance the CDN path exists for. On failure it names both required steps, the orange cloud and the Origin Rule sending the hostname to `PORT_CDN`, and links the CDN section of the troubleshooting guide.
- **`moav user base64` says what it is.** It emits a zipped bundle for moav-client's e2e input, and a release tester reasonably read the name as "the base64 subscription" and pasted it into Streisand, which rejected it. Behaviour is unchanged — the name is what moav-client documents — but `AGENTS.md` had described it as printing the user's *subscription* and `moav help` called it a "quick import". Both corrected, and the command now states the distinction in its help and in the line it prints before you paste.
- **New donation addresses.** Monero and two Lightning options (a BOLT12 offer and a Lightning address) added, and the Bitcoin address replaced with a Taproot one. `.github/FUNDING.yml` stays the single source: both READMEs regenerate from it and the website pulls it on deploy. Each was verified before publishing rather than by eye — the Bitcoin address by computing its bech32m checksum, since a mistyped receiving address loses money silently.
- **XHTTP is donated by default.** Added to the shipped `MAHSANET_PROTOCOLS`; MahsaNet supports it as is.
- **The WireGuard-over-wstunnel QR is gone.** Its config points at `127.0.0.1`, so scanning it into a WireGuard app produced a tunnel that connected and carried no traffic — indistinguishable from the real QR as an image, and the exact shape of a bug report we chased. The `.conf` still ships, with the caveat stated in the guide, and stale copies are removed from regenerated bundles. The WireGuard QR itself was investigated and is correct: its payload is byte-identical to the working `.conf`.

### Security
- **Admin container secrets are out of the container config.** `docker inspect moav-admin` printed 130 environment variables including `ADMIN_PASSWORD`, `REALITY_PRIVATE_KEY` and `MAHSANET_API_KEY`, because the compose service used `env_file: .env`, which bakes every value into `Config.Env`. `admin-entrypoint.sh` already runs as root before dropping to the app user — the same place the Clash API secret is read — so it now loads `.env` itself and exports it into the app process only. `Config.Env` is down to 20 variables with none of the three secrets, while the `ENABLE_*`/`PORT_*` toggles still reach the web-admin add path. Parsed literally rather than sourced, since values legitimately contain `|`.

### Documentation
- **`llms.txt` rewritten** with an operating-essentials section — one-command install, `.env` location, admin dashboard and Grafana URLs, `moav help` — plus grouped links and community entries, so an agent has what it needs before following anything.
- **README corrections.** Reconciled claims against the shipped behaviour, restructured around the philosophy, file layout and protocol table, and added the donation and community sections. Farsi rebuilt to match.

### Testing
- Seven suites added, all wired into CI: DNS-tunnel bundle completeness, subscription-vs-bundle-blob distinction, bundle-guide section visibility across all 14 protocols, `ENABLE_*` gating on both add paths, release-footer generation, `llms.txt` link hygiene (structure plus a networked pass), and the `ENABLE_CDN` truth table including the upgrade case. Two networked gates fetch every link the release footer and `llms.txt` publish.
- e2e now asserts protocol gating end to end on a real server: a normal add has everything enabled, two disabled protocols stay out while a still-enabled one stays in, and donate mode yields only the donated set — via **both** the CLI and the dashboard's invocation shape, which is the path the MahsaNet donation actually uses and the one where donate mode was silently a no-op.
- The e2e gating assertions are derived from the `.env` flags and each protocol's domain requirement rather than hardcoded, so the same steps hold in a domainless run. They were written hardcoded first and failed a domainless pass on the assertions rather than on the code.
- e2e also flips toggles on one server and checks the bundle **both** ways — `ENABLE_CDN` off/on, and XHTTP + MasterDNS + WireGuard off/on, one per generation code path (`scripts/lib/xray.sh`, `user-add.sh`'s DNS blocks, `wg-user-add.sh`). Both directions matter: a gate that hid everything would pass a test that only checks for absence, so each pass also asserts the rest of the bundle survives and that the guide's section visibility follows the artifacts.
...
- **Releases publish `llms.txt` and refresh the docs site automatically.** The AI index ships as a release asset, and tagging a release now dispatches the moav-site deploy, so `moav.sh/install.sh` and the docs go live within minutes of a tag instead of waiting on a manual step.
