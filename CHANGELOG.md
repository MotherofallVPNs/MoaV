...
- **User bundles are no longer world-writable (or world-readable).** `outputs/bundles` and `state/users` were held together with `chmod 777` / `chmod -R a+rwX` so the non-root admin app and the root-run provisioning paths could both write — leaving every user's WireGuard private keys and share URIs readable *and modifiable* by any local account. The admin user is now pinned to uid 2000, root-run paths chown to it (`grant_admin_rw`), and every world bit is stripped; the admin entrypoint repairs existing installs on each start. `configs/monitoring` deliberately keeps world-read (grafana and prometheus run as non-root uids). Non-root operators who read bundles directly now need `sudo`.
