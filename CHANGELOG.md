...
- **`moav test generate <user>`** — emits base64 of a **text-only** bundle (the config files + `subscription.txt`; excludes the QR PNGs and `README.html`, which are the bulk). Paste it into the `moav-client` e2e's `bundle_b64` dispatch input to validate a deployed server against the current client, or use it for a quick client import (`moav test generate alice | pbcopy`).
